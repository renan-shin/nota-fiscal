...
try:
    ...
    print("Objeto NF-e carregado com sucesso!")
...
def temBolSemRio(nfe, empresa):
    ...
    if int(nfe.Pedido) < 100000000 and int(nfe.Pedido) > 0:
        cursor.execute("SELECT * FROM GreenMotor.dbo.Pagamentos WHERE CodPedido = %s AND Forma = %s AND Conta NOT LIKE %s", [nfe.Pedido, 'BOL', '%Rio%',])
    else:
        cursor.execute("SELECT * FROM Lanmax.dbo.Pagamentos WHERE CodPedido = %s AND Forma = %s AND Conta NOT LIKE %s", [nfe.Pedido, 'BOL', '%Rio%',])
...
def temBol(nfe, empresa):
    ...
    if int(nfe.Pedido) < 100000000 and int(nfe.Pedido) > 0:
        ...
        cursor.execute("SELECT * FROM Lanmax.dbo.Pagamentos WHERE CodPedido = %s AND Forma = %s", [nfe.Pedido, 'BOL',])
...
def dest(nfe, empresa):
    ...
    return obj_nfe_util.destinatario310(
        ...
        nfe.dest_xLgr.replace('&', '&amp;') if nfe.dest_xLgr else '',
...
def envia_gnre(nfe, empresa, receita):
    ...
    if Path(caminho_gnre).is_dir():
        if Path(arquivo_gnre).is_file():
            ...
            resultado = obj_nfe_util.EnviaGNRE(empresa.IdentificacaoAmbiente, empresa.Certificado, '2.15', xml_gnre, msg_retws, msg_resultado, nro_recibo, dh_recibo, t_est_proc, proxy, usuario, senha)
...
def busca_gnre(request, nfe, empresa, nro_recibo, receita):
    ...
    resultado = obj_nfe_util.BuscaGNRE(empresa.IdentificacaoAmbiente, empresa.Certificado, '2.15PDF_ARQPGTO', msg_dados, msg_retws, nro_recibo, status, msg_resultado, proxy, usuario, senha)
...
def gera_chave_acesso_por_periodo(empresa, ide_mod, num_nfe, mes, ano):
    status = 0
    msg_resultado = ''
    cnf = ''
    cdv = ''
    ...
    return status, msg_resultado, chave_nfe