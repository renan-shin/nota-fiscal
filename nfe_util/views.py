...
def index(request):
    ...
    return render(request, 'nfe_util/index.html', {'empresas': empresas})

@login_required
def edit(request, empresa_filial, id_nfe):
    try:
        ...
        return redirect('nfe_util_index')
    ...
    if request.method == 'POST':
        ...
        entrega_bairro = request.POST.get('bairro_entrega', '')
        chave_ref = request.POST.get('chave-ref', '')
        ...
        if nfe.status_sefaz != 'NFe não enviada' and int(nfe.ide_mod) == 55 and opcao_envio_email not in('0','1','2','3'):
            ...
            return render(request, 'nfe_util/edit.html', {
                'nfe': nfe,
                'nfe_itens': nfe_itens,
                'empresa': empresa,
                'formas_pagto': formas_pagto,
        ...
        if result[0] != 0:
            ...
            if nfe.status_sefaz == 'NFe não enviada':
                ...
                nfe.entrega_xBairro = entrega_bairro
                nfe.xJust = xjust
                nfe.ide_NFRefs = chave_ref
        else:
            if nfe.status_sefaz == 'NFe não enviada':
                nfe.dest_eMail = email_cliente
                nfe.dest_nro = numero_dest
...
def monitoramento_nfe(request):
    ...
    return render(request, 'nfe_util/monitoramento.html', {'nfes': nfes})
...
def email(request):
    ...
    return render(request, 'nfe_util/email.html', {'nfes_enviar': nfes_enviar, 'nfes_erro': nfes_erro})
...
def faturados(request):
    ...
    if request.method == 'GET':
        return render(request, 'nfe_util/faturados.html', {'empresas': empresas, 'data_atual': data_atual})
        ...
        if empresa_filial == '':
            ...
            return render(request, 'nfe_util/faturados.html', {'empresas': empresas, 'faturamento': faturamento, 'data_atual': data_atual})
        ...
        try:
            ...
            messages.error(request, 'Empresa não encontrada!')
            return redirect('nfe_util_faturados')
...
def retransmitir(request, empresa_filial, id_nfe):
    try:
        ...
        return redirect('nfe_util_monitoramento_nfe')
...
@login_required
def gerar_chave_acesso(request):
    empresas = Empresa.objects.all().values('EmpresaFilial', 'Mnemonico').order_by('Mnemonico')

    if request.method == 'GET':
        return render(request, 'nfe_util/chave-acesso.html', {'empresas': empresas})
    elif request.method == 'POST':
        empresa_filial = request.POST.get('empresa', '')
        ide_mod = request.POST.get('modelo', '')
        num_nfe = request.POST.get('numero', '')
        mes = request.POST.get('mes', '')
        ano = request.POST.get('ano', '')

        if empresa_filial == '':
            messages.error(request, 'Empresa não encontrada!')
            return redirect('nfe_util_gerar_chave_acesso')
        ...
        if ide_mod not in('55', '65'):
            messages.error(request, 'Modelo da NF-e inválido!')
            return redirect('nfe_util_gerar_chave_acesso')
        
        num_nfe = str(num_nfe).zfill(9)
        mes = mes.zfill(2)

        print(empresa, ide_mod, num_nfe, mes, ano)

        status, msg_resultado, chave_nfe = gera_chave_acesso_por_periodo(empresa, ide_mod, num_nfe, mes, ano)

        return render(request, 'nfe_util/chave-acesso.html', {'empresas': empresas, 'status': status, 'msg_resultado': msg_resultado, 'chave_nfe': chave_nfe})